...
const Card = ({ details, type, theme }: Props) => {
    ...
    if (type === 'course') {
        ...
        return (
            <div
                className={cn('p-4 mx-4 h-[400px] rounded-[2rem] bg-pink flex flex-col gap-3 shadow-card min-h-[34.375rem]', {
                    'bg-black': isBlack,
                })}>
                <div className='flex-[70%] min-h-[70%] relative image-fade'>
                    {Number(details.discount) > 0 ? (
                        <div className='absolute -left-3 -top-2 discount-bg p-2 flex justify-center items-center'>
                            <span className='text-white px-1'>%{usePersianNums(details.discount)}</span>
                        </div>
                    ) : null}
                    <img className='w-full h-full rounded-3xl' src={`${BASE_URL}${details.images[1]}`} alt='header-image' />
                    <OwnerProfile
                        name='صابر زارعی'
                ...
                <div className='flex-[30%] min-h-[30%] py-2 flex flex-col justify-between'>
                    <div className='flex flex-col gap-2'>
                        ...
                        {Number(details.discount) > 0 ? (
                            <span
                                className={cn('text-black text-right line-through opacity-80 text-sm', {
                                    'text-white': isBlack,
                                })}>
                                {usePersianNums(details.price, true)} تومان
                            </span>
                        ) : null}
                        <span className={cn('text-black text-right', { 'text-white': isBlack })}>
                            {/* {usePersianNums(details.finalPrice, true)} تومان */}
                            <BuyText className={cn({ 'text-white': isBlack })} />
                        </span>
                    </div>
                    <div className='flex items-center justify-between'>
                        <Link to={url} className='w-[80%] [@media(max-width:630px)]:w-[70%]'>
                            <MainButton intent={isBlack ? 'white' : 'black'} size='card' text='بیشتر بدانید' />
                        </Link>
                        {products.find(item => item._id === details._id) ? null : (
                            <BuyComp intent={isBlack ? 'white' : 'black'} size='small' onClick={buyHandler} />
                        )}
                    </div>
                </div>
            </div>
        ...
        const [finalPrice, setFinalPrice] = useState<number | string>(details.finalPricePhysical);
        const { activeTab, handleClick } = useCurrentTab(0);

        const clickHandler = (finalPrice: number | string, price: number | string, idx: number) => {
            setPrice(price);
                ...
                <div className='flex flex-col gap-2 items-center'>
                    <Link to={url}>
                        <figure className='w-28 h-40 transition-all duration-300 hover:scale-110 relative'>
                            ...
                            <img className='w-full h-full rounded-2xl' src={`${BASE_URL}${details.images[0]}`} alt='book-img' />
                        </figure>
                    </Link>
                ...
                <div className='flex flex-col gap-2 items-center'>
                    <div className='flex gap-2 items-center'>
                        <IconWrapper
                            ...
                            title='نسخه فیزیکی'
                            onClick={() => clickHandler(details.finalPricePhysical, details.pricePhysical, 0)}>
                            <BookIcon
                        ...
                        <IconWrapper
                            ...
                            title='نسخه دیجیتالی'
                            onClick={() => clickHandler(details.finalPriceVirtual, details.priceVirtual, 1)}>
                            <Pdf
                                id='book-pdf'
                                className={cn('w-4 h-4 fill-white', {
                    ...
                    {Number(details.discount) > 0 ? (
                        <span className='line-through opacity-50 text-xs'>{usePersianNums(price, true)} تومان</span>
                    ) : null}
                    <span className='font-bold text-purple'>{usePersianNums(finalPrice, true)} تومان</span>
                </div>
                <Link to={url}>
                    <MainButton className='self-center w-full max-w-fit shadow-none' text={`مشاهده بیشتر`} intent='purple' size='medium' />
                </Link>
            </div>
        );
        ...
        return (
            <div className='flex gap-4 h-full bg-lowPink p-4 rounded-2xl shadow-main subHero:flex-col'>
                <figure className='w-[21rem] h-48 subHero:self-center subHero:w-[70%]'>
                    <img src={`${BASE_URL}${details.images[0]}`} alt='article-img' className='w-full h-full rounded-2xl' />
                </figure>
            ...
            return (
                <div className='flex gap-4 h-[10.5rem] bg-lowPink p-4 rounded-2xl shadow-main text-black tips2:text-xs'>
                    <figure className='w-48 h-full relative tips2:w-20 tips2:h-20'>
                        ...
                        <img src={`${BASE_URL}${details.images[0]}`} alt='course-img' className='w-full h-full rounded-2xl' />
                    </figure>
                    <div className='flex flex-col gap-3 w-full justify-between text-sm tips2:text-xs'>
                        <div className='flex flex-col gap-4'>
                            ...
                            <div className='flex justify-between items-center'>
                                <span>قیمت :</span>
                                <span className='line-through'>{usePersianNums(details.pricePhysical, true)} تومان</span>
                                <span>{usePersianNums(details.finalPricePhysical, true)} تومان</span>
                            </div>
                        </div>
                        <div className='flex gap-2 items-center self-end'>
                            ...
                            {products.find(item => item._id === details._id) ? null : (
                                <BuyComp intent={'black'} size='small' onClick={() => buyHandler('book')} />
                            )}
                        </div>
                    </div>
                </div>
            ...
            return (
                <div className='flex gap-4 h-[10.5rem] bg-lowPink p-4 rounded-2xl shadow-main text-black tips2:text-xs'>
                    <figure className='w-48 h-full relative tips2:w-20 tips2:h-20'>
                        ...
                        <img src={`${BASE_URL}${details.images[0]}`} alt='course-img' className='w-full h-full rounded-2xl object-cover' />
                    </figure>
                    <div className='flex flex-col gap-3 w-full justify-between text-sm tips2:text-xs'>
                        <div className='flex flex-col gap-4'>
                            <h1 className='font-bold'>{details.title}</h1>
                            <div className='flex justify-between items-center'>
                                <span>قیمت :</span>
                                {Number(details.discount) > 0 ? (
                                    <span className='line-through'>{usePersianNums(details.price, true)} تومان</span>
                                ) : null}
                        ...
                        <div className='flex gap-2 items-center self-end'>
                            ...
                            {products.find(item => item._id === details._id) ? null : (
                                <BuyComp intent={'black'} size='small' onClick={() => buyHandler('course')} />
                            )}
                        </div>
                    </div>
                </div>
