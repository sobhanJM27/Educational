...
const Course = () => {
    ...
    return (
        <WithLoaderAndError {...{ data, isLoading, isError, error }}>
            {data ? (
                <>
                    <SeoTags titleTemplate={`${data?.title}`} description={`صفحه ی دوره  ${data.title}`} Url={window.location.href}>
                        <meta property='og:title' content={`${data?.title}`} />
                        <meta property='og:description' content={`صفحه ی دوره  ${data.title}`} />
                        <meta property='og:image' content={`${BASE_URL}${data.images[1]}`} />
                        <meta property='og:url' content={window.location.href} />
                    </SeoTags>
                    <main className='flex flex-col'>
                        <ParallaxHeader bg={`${BASE_URL}${data.images[0]}`} className='flex items-center'>
                        ...
                        <CourseInfo
                            ...
                            ref={infoRef}
                            type='course'
                            isInBasket={products.find(item => item._id === data._id) ? true : false}
                        ...
                        <div className='p-4 z-10' id='comments' ref={commentsef}>
                            <SumComments type='course' comments={data.comments} id={data._id} />
                        </div>
                        <ProductsSlider
                            apiUrl=''
